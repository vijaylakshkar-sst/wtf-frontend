...
import type { MasterConfig, MasterField, MasterRecord } from "@/components/masters/types";
...
export function MasterCrudPage({ config, Shell }: { config: MasterConfig; Shell: ComponentType<ShellProps> }) {
    ...
    <Shell>
      <section className="builder-main masters-main">
        ...
        <section className="masters-stats" aria-label={`${config.title} summary`}>
          ...
          <article><small>Active</small><strong>{activeCount}</strong><span>Available in supplier workflows</span></article>
          <article><small>Inactive</small><strong>{inactiveCount}</strong><span>Hidden from new product setup</span></article>
        </section>
          ...
          <section className="master-form-panel">
            <header>
              <div>
                ...
                <p>{editingId ? "Update details and save changes." : "Add a master value for supplier product management."}</p>
              </div>
